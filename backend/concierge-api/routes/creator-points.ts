...
import { getCreatorStanding, publicCreatorPointsMeta } from "../creator-points";
...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    const standing = await getCreatorStanding(wallet);

    return new Response(
      JSON.stringify({ ok: true, ...standing, meta: publicCreatorPointsMeta() }),
      {
        status: 200,
