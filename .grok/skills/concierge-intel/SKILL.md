...
description: Call Concierge Agent paid market intelligence on conc-exe.xyz — DeFi TVL, yields, whales, wallet, verdict, alpha desks, scalp desk, and Gemini chat. Use when the user asks for live market intel, x402 micropayments, or Concierge API integration. Requires pay CLI for automatic 402 settlement.
---
...
| `POST /api/concierge-intel-*` (9 intel desks) | $0.10 each |
| `POST /api/news-open` | $0.10 |
| `POST /api/lounge-signal-open` | $0.10 |
| `POST /api/lounge-signal-publish` | $1.00 |

Full catalog: `/agent/endpoints` · Docs: `/docs/grok-build`
